...
void Scheduler::start()
{
    ...
    if (started_ || stopRequested_) {
        throw std::logic_error(
            "Scheduler can only be started once, before shutdown");
    }
...
Scheduler::Clock::time_point Scheduler::calculateNextRun(
...
void Scheduler::schedulerLoop()
{
    try {
        ...
        while (!stopRequested_) {
            if (tasks_.empty()) {
                scheduleChanged_.wait(lock, [this] {
            ...
            scheduleChanged_.wait_until(lock, deadline, [this] {
                return stopRequested_;
            });
            ...
            for (auto& scheduled : tasks_) {
                ...
                scheduled.nextRun =
                    calculateNextRun(scheduled.task, now);
            }
        }
    }
    ...
    try {
        while (true) {
            ...
            try {
                ...
                CHORED_LOG_ERROR(
                    "Task ", task.name, " failed: ", error.what());
            }
