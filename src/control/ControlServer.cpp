...
{
  const auto active = scheduler.activeTask();
  if (!active)
  ...
  out << "TASK\tPID\tRUNNING FOR\n"
      << std::quoted(printable(active->name)) << '\t' << active->pid << '\t' << seconds / 60 << "m "
      << seconds % 60 << "s\n";
  return out.str();
}
} // namespace
...
void ControlServer::loop() noexcept
{
  ...
  for (;;)
  {
    ...
    if (getsockopt(client.value, SOL_SOCKET, SO_PEERCRED, &peer, &size) != 0 ||
        peer.uid != getuid())
      continue;
