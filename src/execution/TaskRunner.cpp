...
TaskResult TaskRunner::run(const TaskConfig &task) const
{
  ...
  checkSpawnError(posix_spawnattr_setsigmask(&attributes.value, &signalMask),
  ...
  checkSpawnError(posix_spawnattr_setpgroup(&attributes.value, 0),
                  "Cannot configure child process group");

  const short flags = POSIX_SPAWN_SETSIGMASK | POSIX_SPAWN_SETSIGDEF | POSIX_SPAWN_SETPGROUP;

  checkSpawnError(posix_spawnattr_setflags(&attributes.value, flags),
  ...
  // Prepare potentially allocating state before starting the process.
  ...
  // Release before constructing the cleanup guard to avoid nested locking.
  activeLock.unlock();
  ResetActive reset{activeMutex_, active_};
  activeLock.lock();
